...
**Note:** You might notice that certain attributes and methods start with a single underscore (`_`). By convention in Python, attributes and methods starting with `_` are considered internal to the implementation and thus not to be used directly by code outside of the implementation.
