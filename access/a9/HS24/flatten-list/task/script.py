...
def flatten_list(nested_list):
    ...
    # if there is no list elemnnt in the list, return it 

    if not any(isinstance(sub, list) for sub in nested_list):
        return nested_list

    if type(nested_list) != list:
        raise TypeError("Expected argument has to be a list.")

    list_without_nests = [element for element in nested_list if type(element) != list]

    
    return list_without_nests + nested_list[1]
